import { createChart, delteProductFromChart, getChart, syncQuantity, } from "./service.js";
...
const getChartController = async (req, res) => {
    try {
        const { userId } = getAuth(req);
        if (!userId) {
            return res.status(401).json({
        ...
        const transformedChart = {
            ...
            products: chart.products.map((item) => ({
                ...
                product: {
                    ...item.product,
                    typesChoose: item.product.typesChoose.map((filename) => filename ? `${host}/images/${filename}` : null),
                },
            })),
        };
...
const deleteProductFromChartController = async (req, res) => {
    try {
        ...
        const chartProduct = await delteProductFromChart(userId, +chartId, +idProduct);
...
const syncQuantityController = async (req, res) => {
    try {
        ...
        const updatedChart = await syncQuantity(+chartProductId, +quantity, opreationType, userId);
        return res.status(200).json({
            data: updatedChart,
        ...
        return res.status(500).json({
            message: error instanceof Error ? error.message : "Error updating quantity",
        });
    }
};
export { createChartController, getChartController, deleteProductFromChartController, syncQuantityController, };
