...
const create = async (name, price, description, quantity, categoryName, sizeValues, typesChoose = {}) => {
...
const addProduct = async (userId, productId, size, image, quantity = 1) => {
    try {
        ...
        const existProductInChart = activeChart.products.find((product) => product.product.id === productId);
        if (existProductInChart) {
            const updateChart = await prisma.chart.update({
                ...
                data: {
                    totalPayment: activeChart.totalPayment + existProductInChart.product.price,
                    orderAmount: activeChart.orderAmount + existProductInChart.product.price,
                },
            });
            const existingColors = ensureArray(existProductInChart.selectedColors);
            const updatedColors = Array.from(new Set([...existingColors, image]));
            const existingSizes = ensureArray(existProductInChart.selectedSizes);
        ...
        const updateChartForNewProduct = await prisma.chart.update({
            ...
            data: {
                totalPayment: activeChart.totalPayment + newProductInChart.product.price,
...
const delteAddProduct = async (idProduct, idUser) => {
