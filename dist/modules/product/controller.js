...
export const getPublickKey = async (req, res) => {
  try {
...
const getAllProductsController = async (req, res) => {
  ...
  try {
    ...
    const result = products.map((p) => ({
      ...
      name: p.name,
      price: p.price,
      typesChoose: p.typesChoose.map((filename) =>
...
const getProductByIdController = async (req, res) => {
  ...
  try {
    ...
    const result = {
      ...Product,
      typesChoose: (Product?.typesChoose).map((filename) =>
        filename ? `${host}/images/${filename}` : null,
      ),
    };
    return res.status(200).json(result);
...
const addProductToChartController = async (req, res) => {
  try {
    const { userId } = getAuth(req);
    if (!userId) {
      return res.status(401).json({ error: "User not authenticated" });
...
const deleteFromChartController = async (req, res) => {
  try {
